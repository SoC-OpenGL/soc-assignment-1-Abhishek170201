...
int main(int argc, char** argv)
{
 ...
 // glfwWindowHint(GLFW_OPENGL_FORWARD_COMPAT, GL_TRUE);
  ...
  unsigned char *data=stbi_load("starwars.jpg",&width,&height,&nrChannels,0);
  ...
  vertices[0]=0.0f;
    ...
    {
        vertices[5*i]=radius*cos(angle*i)*fabs(cos(angle*i));
        vertices[5*i+1]=radius*sin(angle*i)*fabs(cos(angle*i));
        vertices[5*i+2]=0.0f;
        vertices[5*i+3]=5*radius*cos(angle*i)*cos(angle*i);
        vertices[5*i+4]=5*radius*sin(angle*i)*sin(angle*i);
    }
  //Ask GL for a Vertex
